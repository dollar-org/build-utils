...
then
    ...
    export NUMERIC_RELEASE=$(printf "%x" $CIRCLE_BUILD_NUM)
else
...
s3_deploy() {
    ...
    then
        ...
        aws s3 cp --quiet --cache-control "max-age=0, no-cache, no-store, private" out/redirect-expanded.html s3://${DEPLOY_HOST}/index.html
    else
