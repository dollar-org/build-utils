...
s3_deploy() {
    ...
    then
        ...
        aws s3 sync  --delete --cache-control "max-age=0, no-cache, no-store, private" --expires "" --exclude "*.ts"  --exclude "*assets/scss/*" --exclude "*typings/*"   out/ s3://${DEPLOY_HOST}/${environment}/
    fi
